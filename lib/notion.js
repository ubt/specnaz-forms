...
// Заменить функцию listEvaluateesForReviewerUser в lib/notion.js
...
export async function listEvaluateesForReviewerUser(userId) {
  ...
  try {
    ...
    if (!allFilters.length) {
      ...
      console.log('[EVALUATEES] Expected fields:', { 
        selfScorer: PROP.selfScorer,
        p1Peer: PROP.p1Peer, 
        p2Peer: PROP.p2Peer,
        managerScorer: PROP.managerScorer,
    ...
    // Выполняем поиск с каждым фильтром отдельно для лучшей диагностики
    let allRows = [];
    for (let i = 0; i < allFilters.length; i++) {
      const filter = allFilters[i];
      console.log(`[EVALUATEES] Executing filter ${i + 1}/${allFilters.length}:`, JSON.stringify(filter, null, 2));
      ...
      try {
        ...
        console.log(`[EVALUATEES] Filter ${i + 1} found ${filterRows.length} rows`);
        allRows.push(...filterRows);
      } catch (filterError) {
        console.error(`[EVALUATEES] Filter ${i + 1} failed:`, filterError.message);
      }
    }
    ...
    if (!uniqueRows.length) {
      ...
      // Дополнительная диагностика - проверяем есть ли вообще записи в матрице
      try {
        ...
        if (sampleRows.length > 0) {
          ...
          console.log(`[EVALUATEES] Sample row structure:`, {
            ...
            selfScorer: sampleRow.properties?.[PROP.selfScorer]?.people?.map(p => p.id) || 'empty',
            p1Peer: sampleRow.properties?.[PROP.p1Peer]?.people?.map(p => p.id) || 'empty',
            p2Peer: sampleRow.properties?.[PROP.p2Peer]?.people?.map(p => p.id) || 'empty',
            managerScorer: sampleRow.properties?.[PROP.managerScorer]?.people?.map(p => p.id) || 'empty',
            employee: sampleRow.properties?.[PROP.employee]?.relation?.map(r => r.id) || sampleRow.properties?.[PROP.employee]?.people?.map(p => p.id) || 'empty'
          });
        }
      } catch (sampleError) {
