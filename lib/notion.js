...
export async function fetchEmployeeSkillRowsForReviewerUser(employees, reviewerUserId) {
  console.log(`[SKILLS] Starting optimized skill loading for ${employees.length} employees`);
  ...
  try {
    ...
    const allRows = await queryAllPages({
      database_id: MATRIX_DB_ID,
      filter: allFilters.length === 1 ? allFilters[0] : { or: allFilters },
      page_size: 100
    });

    // ОПТИМИЗАЦИЯ 2: Собираем все уникальные skill IDs
    ...
    for (const row of allRows) {
      ...
      // Группируем строки по сотрудникам
      const empId = /* логика получения employee ID */;
      if (!rowsByEmployee.has(empId)) {
        rowsByEmployee.set(empId, []);
      }
      rowsByEmployee.get(empId).push(row);
    }

    // ОПТИМИЗАЦИЯ 3: Параллельная загрузка информации о навыках
    const skillInfoMap = new Map();
    const skillIds = Array.from(allSkillIds);
    
    // Загружаем навыки батчами параллельно
    const BATCH_SIZE = 20;
    for (let i = 0; i < skillIds.length; i += BATCH_SIZE) {
      const batch = skillIds.slice(i, i + BATCH_SIZE);
      const batchPromises = batch.map(skillId => 
        loadSkillInformation(skillId).then(info => ({ skillId, info }))
      );
      
      const batchResults = await Promise.all(batchPromises);
      batchResults.forEach(({ skillId, info }) => {
        skillInfoMap.set(skillId, info);
      });
    }

    // Теперь собираем результат используя заранее загруженные данные
    const result = [];
    for (const employee of employees) {
      const rows = rowsByEmployee.get(employee.employeeId) || [];
      // ... обработка с использованием skillInfoMap
    }
