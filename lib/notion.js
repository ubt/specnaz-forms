...
export const PROP = {
  employee: "Сотрудник",
  p1: "P1_peer",              // People field для P1 peer
  p2: "P2_peer",              // People field для P2 peer  
  manager: "Manager_scorer",   // People field для Manager
  skill: "Навык",             // Relation на навыки
  skillDesc: "Навык",         // Если описание в relation
  comment: "Комментарий",     // Rich text поле
  // БД Сотрудники
  team: "Команда",
  empAccount: "Учетка",
  empTitle: "Сотрудник",
...
export function computeRoleOnRow(row, reviewerCtx, matrixProps) {
  ...
  const pplHas = n => {
   ...
   if (has("Self_scorer")) {
	if (isPeople("Self_scorer") && pplHas("Self_scorer")) return "self";
	}
...
const nameConcurrency = Number(process.env.NAME_CONCURRENCY ?? 10) || 10;
  let idx = 0;
