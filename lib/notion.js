...
const cache = new Map();
const CACHE_TTL = 30 * 60 * 1000; // 10 минут для навыков
...
// Кэшируйте результаты на уровне функций
export async function getCachedEmployeeSkills(employeeId, reviewerUserId) {
  const cacheKey = `skills_${employeeId}_${reviewerUserId}`;
  const cached = getCached(cacheKey);
  if (cached) return cached;
  
  const result = await fetchEmployeeSkillRowsForReviewerUser(...);
