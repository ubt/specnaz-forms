...
// Список оцениваемых для ревьюера с исправленной логикой поиска
export async function listEvaluateesForReviewerUser(userId) {
  ...
  // Разделяем поиск по типам полей
  const peopleFilters = [
  ...
  const relationFilters = [
    ...buildFilters(PROP.employee), // Это relation поле
  ];

  const allFilters = [...peopleFilters, ...relationFilters];

  console.log(`[EVALUATEES] Built ${allFilters.length} filters:`, allFilters);
  ...
  console.log(`[EVALUATEES] Querying matrix database with ${allFilters.length} OR filters...`);
  
  // Делаем запрос с увеличенным page_size для получения всех записей
  ...
  console.log(`[EVALUATEES] Found ${allRows.length} matrix rows total`);

  const employeeMap = new Map();
  const reviewerCtx = { userId, pageIds: myPageIds };

  for (const [index, row] of allRows.entries()) {
    console.log(`[EVALUATEES] Processing row ${index + 1}/${allRows.length}: ${row.id}`);
...
// Исправленная функция загрузки навыков с улучшенной обработкой
export async function fetchEmployeeSkillRowsForReviewerUser(employees, reviewerUserId) {
  ...
  console.log(`[SKILLS] Employee property type: ${empDef?.type}, processing ${employeeIds.length} employee IDs`);
  ...
  if (!orFilters.length) {
    console.warn('[SKILLS] No valid employee filters created');
    return [];
  }

  console.log(`[SKILLS] Querying matrix with ${orFilters.length} employee filters...`);
  
  // Используем queryAllPages для получения ВСЕХ записей
  const allRows = await queryAllPages({
    database_id: MATRIX_DB_ID,
    filter: { or: orFilters },
    page_size: 100
  ...
  console.log(`[SKILLS] Found ${allRows.length} total matrix rows for all employees`);

  // Batch-загрузка всех навыков одним запросом
  console.log(`[SKILLS] Loading skills data...`);
  const skillsMap = await batchLoadSkillsData(allRows);
  console.log(`[SKILLS] Loaded ${skillsMap.size} unique skills`);
  ...
  for (const employee of employees) {
    ...
    // Фильтруем строки для конкретного сотрудника
    ...
    if (!employeeRows.length) {
      console.warn(`[SKILLS] No rows found for employee ${employee.employeeName}`);
      continue;
    }

    // Определяем роль ревьюера для этого сотрудника
    let role = employee.role; // Используем роль из предыдущего этапа
    
    // Если роль не определена, пытаемся вычислить
    if (!role) {
    ...
    if (!role) {
      console.warn(`[SKILLS] No role determined for employee ${employee.employeeName}`);
      continue;
    }

    console.log(`[SKILLS] Role for ${employee.employeeName}: ${role}`);
    ...
    for (const [index, row] of employeeRows.entries()) {
      ...
      console.log(`[SKILLS] Processing skill row ${index + 1}/${employeeRows.length} for ${employee.employeeName}`);
      ...
      if (!skillData) {
        console.warn(`[SKILLS] Skill data not found for ID ${skillId}`);
        // Пытаемся загрузить навык отдельно
        try {
          const skillPage = await notionApiCall(() => notion.pages.retrieve({ page_id: skillId }));
          const skillName = getTitleFromProps(skillPage.properties || {}) || "Навык";
          skillsMap.set(skillId, { name: skillName, description: "" });
          console.log(`[SKILLS] Loaded missing skill: ${skillName}`);
        } catch (error) {
          console.error(`[SKILLS] Failed to load skill ${skillId}:`, error.message);
          skillsMap.set(skillId, { name: "Неизвестный навык", description: "" });
        }
      }
      
      const finalSkillData = skillsMap.get(skillId) || { name: "Навык", description: "" };
      
      items.push({ 
        pageId: row.id, 
        name: finalSkillData.name,
        description: finalSkillData.description,
        current,
        comment 
      });
      
      console.log(`[SKILLS] Added skill: ${finalSkillData.name} (current: ${current})`);
    }
...
async function batchLoadSkillsData(rows) {
  ...
  const BATCH_SIZE = 5; // Еще меньше для надежности в Edge Runtime
