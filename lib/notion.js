...
const limit = pLimit(2); // Консервативный лимит для Notion API
...
async function loadSkillInformation(pageId) {
  const page = await notion.pages.retrieve({ page_id: pageId });
  let skillRollup = page.properties['Описание навыка'].rollup;
  
  // Проверяем, нужна ли пагинация для больших rollup
  if (skillRollup.array && skillRollup.array.length >= 25) {
    try {
      const fullProperty = await notion.pages.properties.retrieve({
        page_id: pageId,
        property_id: page.properties['Описание навыка'].id
      });
      
      // Обрабатываем пагинацию если необходимо
      let allResults = fullProperty.results || [];
      let cursor = fullProperty.next_cursor;
      
      while (cursor) {
        const nextPage = await notion.pages.properties.retrieve({
          page_id: pageId,
          property_id: page.properties['Описание навыка'].id,
          start_cursor: cursor
        });
      ...
      skillRollup = { ...skillRollup, array: allResults };
    } catch (error) {
      console.warn('Fallback to direct skill queries:', error);
      return await loadSkillsDirect(page.properties['Навыки']?.relation || []);
    }
  }
  
  return processSkillData(skillRollup);
}

// ИСПРАВЛЕННАЯ основная функция загрузки навыков
async function fetchEmployeeSkillRowsForReviewerUser(reviewerId) {
  const employeeQuery = await notion.databases.query({
    database_id: EMPLOYEE_SKILLS_DB_ID,
    filter: {
      property: "Reviewer",
      people: { contains: reviewerId }
    }
  });
  
  // Загружаем связанные навыки с контролем concurrency
  const skillsData = await Promise.all(
    employeeQuery.results.map(row =>
      limit(async () => {
        try {
          return await loadDetailedSkillInfo(row);
        } catch (error) {
          console.warn(`Failed to load skill for row ${row.id}:`, error);
          return { ...row, skill_details: null };
        }
      })
    )
  );
  
  return skillsData.filter(item => item.skill_details !== null);
}
