    ...
    for (const employee of employees) {
      ...
      try {
        ...
        for (const row of relevantRows) {
          ...
          if (!uniqueSkills.has(skillId)) {
            ...
            uniqueSkills.set(skillId, {
              ...
              current,
              field,
              matrixRowProps: props // Передаем свойства строки матрицы
            });
          }
        }
        ...
        for (const skillEntry of skillEntries) {
          try {
            // Передаем свойства строки матрицы для извлечения описания
            ...
            // Небольшая пауза между запросами только если идем к Notion API
            if (skillInfo.name.includes("Навык (ID:")) {
