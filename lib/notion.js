  ...
  try {
    ...
    for (const employee of employees) {
      ...
      try {
        ...
        // Упрощенный сбор навыков - используем описание из матрицы
        const items = [];
        ...
        for (const row of relevantRows) {
          try {
            ...
            // Получаем описание навыка из матрицы
            const skillDescription = props[PROP.skillDescription]?.rich_text?.map(t => t.plain_text).join("") || "";
            ...
            console.log(`[SKILLS] Added skill: ${skillName} (current: ${current})`);
            
          } catch (skillError) {
            console.error(`[SKILLS] Error processing skill row ${row.id}:`, skillError.message);
            continue;
          }
        }
