...
// LRU Cache для Edge Runtime с автоматическим TTL и eviction
const cache = createCache('default'); // 100 items, 10 min TTL
...
// Rate limiting and API calls are now handled by lib/notionRateLimit.js
// Import: notionApiCall from "./notionRateLimit"
// This consolidates all rate limiting logic in one place
...
async function loadSkillInformation(skillId, matrixRowProps) {
  try {
    ...
    const result = {
	  name: `Навык ${skillId.substring(skillId.length - 8)}`,
