...
// Улучшенный rate limiter с адаптивными задержками
class AdaptiveRateLimit {
  ...
  async wait() {
    ...
    const now = Date.now();
    const recentRequests = this.requests.filter(t => now - t < 1000); // За последнюю секунду
    
    // Если превышен лимит запросов в секунду
    if (recentRequests.length >= this.maxRequestsPerSecond) {
      const oldestRequest = Math.min(...recentRequests);
      const waitTime = 1000 - (now - oldestRequest) + 50; // +50ms буфер
      
      if (waitTime > 0) {
        console.log(`[RATE LIMIT] Waiting ${waitTime}ms due to rate limit`);
  ...
  recordError(error) {
    if (error?.status === 429) {
      ...
      console.warn('[RATE LIMIT] 429 detected, increasing delay');
    }
  }
}

const rateLimit = new AdaptiveRateLimit();

// Обертка для API вызовов с улучшенной retry логикой
async function notionApiCall(apiCall, maxRetries = 3, context = '') {
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      console.error(`[NOTION API] ${context} - Attempt ${attempt + 1} failed:`, error.message);
      ...
      if (error?.status === 400 && error.message?.includes('invalid_json')) {
        console.error(`[NOTION API] Invalid JSON request - check data format`);
        throw error;
      }
...
function safePropertyAccess(page, propertyName, defaultValue = null) {
  try {
    ...
    switch (property.type) {
      ...
      case 'rich_text':
        return property.rich_text?.map(t => t.text?.content).join('') || defaultValue;
      case 'rollup':
        return processRollupProperty(property.rollup) || defaultValue;
      case 'relation':
    ...
    console.warn(`[SAFE ACCESS] Error accessing property ${propertyName}:`, error.message);
    return defaultValue;
  }
}

// Обработка rollup свойств
function processRollupProperty(rollup) {
  ...
  try {
    switch (rollup.type) {
      case 'array':
        ...
        // Извлекаем текст из первого непустого элемента
        for (const item of rollup.array) {
          if (item.title && item.title.length > 0) {
            return item.title.map(t => t.plain_text || t.text?.content).join('');
          }
          if (item.rich_text && item.rich_text.length > 0) {
            return item.rich_text.map(t => t.plain_text || t.text?.content).join('');
          }
        }
        return null;
      ...
      case 'date':
        return rollup.date;
      default:
        return rollup.function === 'show_original' ? rollup : null;
    }
  } catch (error) {
    console.warn('[ROLLUP] Error processing rollup:', error.message);
    return null;
  }
}
...
function getTitleFromProps(props) {
  for (const key in props) {
    ...
    if (v?.type === "title" && v.title?.length) {
      return v.title.map(t => t.plain_text || t.text?.content).join("");
    }
  }
  return null;
}
  ...
  try {
    ...
    if (missing.length > 0) {
      ...
      for (const uid of missing) {
        try {
          const u = await notionApiCall(() => 
            notion.users.retrieve({ user_id: uid }),
            2,
            `Get user ${uid}`
          );
  ...
  try {
    ...
    const res = await notionApiCall(() => 
      notion.databases.query({
        ...
        page_size: 10
      }), 3, `Get employee pages for ${userId}`
    );
      ...
      try {
        ...
        if (empDef?.type === "relation") {
          ...
          if (employeeId && !employeesMap.has(employeeId)) {
            try {
              const page = await notionApiCall(() => 
                notion.pages.retrieve({ page_id: employeeId }),
                2,
...
// ИСПРАВЛЕННАЯ функция загрузки информации о навыке с множественными стратегиями
async function loadSkillInformation(skillId, matrixRowProps) {
  const cacheKey = `skill_${skillId}`;
  const cached = getCached(cacheKey);
  if (cached) {
    console.log(`[SKILL CACHE] Using cached data for skill: ${skillId}`);
    return cached;
  }
  ...
  // Стратегия 1: Проверяем rollup поле в текущей строке матрицы
  if (matrixRowProps && matrixRowProps[PROP.skillDescription]) {
    console.log(`[SKILL LOAD] Trying rollup field strategy`);
    
    const rollupData = processRollupProperty(matrixRowProps[PROP.skillDescription]);
    if (rollupData && rollupData.trim()) {
      skillDescription = rollupData.trim();
      ...
      console.log(`[SKILL LOAD] ✅ Strategy 1 (rollup) succeeded: ${skillName}`);
  ...
  // Стратегия 2: Прямая загрузка страницы навыка
  try {
    console.log(`[SKILL LOAD] Trying direct page load strategy`);
    
    const skillPage = await notionApiCall(() => 
      notion.pages.retrieve({ page_id: skillId }),
      2,
      `Load skill page ${skillId}`
    );
    
    const props = skillPage.properties || {};
    
    // Ищем title поле
    const pageTitle = safePropertyAccess(skillPage, 'Name') || 
                     safePropertyAccess(skillPage, 'Название') ||
                     safePropertyAccess(skillPage, 'Title') ||
    ...
    const descriptionFields = ['Description', 'Описание', 'Content', 'Содержание', 'Details', 'Детали'];
    
    for (const fieldName of descriptionFields) {
      const desc = safePropertyAccess(skillPage, fieldName);
      if (desc && desc.trim()) {
        skillDescription = desc.trim();
        console.log(`[SKILL LOAD] Found description in field: ${fieldName}`);
        break;
      }
    }
    
    // Если описание не найдено, ищем в любых rich_text полях
    if (!skillDescription) {
      for (const [key, value] of Object.entries(props)) {
        if (value?.type === "rich_text" && value.rich_text?.length > 0) {
          ...
          if (text && text.trim() && text.length > 10) {
            ...
            console.log(`[SKILL LOAD] Found description in rich_text field: ${key}`);
            break;
          }
        }
      }
    ...
    console.log(`[SKILL LOAD] ✅ Strategy 2 (direct page) succeeded: ${skillName}`);
    const result = { name: skillName, description: skillDescription };
    setCached(cacheKey, result);
    return result;
    
  } catch (pageError) {
    console.warn(`[SKILL LOAD] Strategy 2 failed - ${pageError.message}`);
  }
  
  // Стратегия 3: Попытка получить данные через property endpoint (для rollup с пагинацией)
  if (matrixRowProps && matrixRowProps[PROP.skillDescription]) {
    try {
      console.log(`[SKILL LOAD] Trying property endpoint strategy`);
      
      const propertyId = matrixRowProps[PROP.skillDescription].id;
      if (propertyId) {
        // Здесь нужен pageId строки матрицы, не навыка
        // Получаем его из контекста вызова
        console.log(`[SKILL LOAD] Property endpoint not available without matrix row page ID`);
      }
    } catch (propertyError) {
      console.warn(`[SKILL LOAD] Strategy 3 failed - ${propertyError.message}`);
    }
  }
  
  // Стратегия 4: Fallback - используем ID как название
  if (skillName === "Неизвестный навык") {
    skillName = `Навык ${skillId.substring(-8)}`;
    console.log(`[SKILL LOAD] ⚠️ Using fallback name: ${skillName}`);
  }
  
  const result = { 
    name: skillName, 
    description: skillDescription || "Описание недоступно" 
  };
...
// УЛУЧШЕННАЯ функция загрузки навыков с защитой от ошибок
  ...
  try {
    ...
    for (const employee of employees) {
      ...
      try {
        ...
        // Загружаем информацию о навыках с защитой от ошибок
        const items = [];
        const skillEntries = Array.from(uniqueSkills.values());
        
        // Последовательная обработка для снижения нагрузки на API
        for (let i = 0; i < skillEntries.length; i++) {
          const skillEntry = skillEntries[i];
          
          try {
            console.log(`[SKILLS] Loading skill ${i + 1}/${skillEntries.length} for ${employee.employeeName}: ${skillEntry.skillId}`);
            ...
            // Добавляем навык с ошибкой, но не прерываем процесс
            items.push({
              pageId: skillEntry.pageId,
        ...
        if (items.length > 0) {
          ...
          const successCount = items.filter(item => !item.name.startsWith('Навык ') && !item.description.startsWith('Ошибка')).length;
          console.log(`[SKILLS] ✅ Added ${items.length} skills for ${employee.employeeName} (${successCount} successful, ${items.length - successCount} with errors)`);
        } else {
    ...
    const successfulSkills = result.reduce((sum, emp) => 
      sum + emp.items.filter(item => !item.name.startsWith('Навык ') && !item.description.startsWith('Ошибка')).length, 0);
    
    console.log(`[SKILLS] Total skills loaded: ${totalSkills}, successful: ${successfulSkills}, errors: ${totalSkills - successfulSkills}`);
