...
export async function fetchEmployeeSkillRowsForReviewerUser(employees, reviewerUserId) {
  ...
  try {
    ...
    // **Оптимизация:** один запрос для получения всех строк по всем сотрудникам
    let allRows = [];
    if (employees.length > 0) {
      const empDef = matrixProps[PROP.employee];
      if (empDef?.type === "relation") {
        // Формируем OR-фильтр по всем pageId сотрудников
        const orFilters = employees.map(e => ({
          property: PROP.employee,
          relation: { contains: e.employeeId }
        }));
        const combinedFilter = orFilters.length === 1 ? orFilters[0] : { or: orFilters };
        allRows = await queryAllPages({
          database_id: MATRIX_DB_ID,
          filter: combinedFilter,
          page_size: 100
        ...
        // Формируем OR-фильтр по всем userId сотрудников
        const orFilters = employees.map(e => ({
          property: PROP.employee,
      ...
      console.log(`[SKILLS] Query for all employees returned ${allRows.length} rows total.`);
    }
    ...
    for (const employee of employees) {
      ...
      try {
        // Фильтруем строки для текущего сотрудника из общего списка
        ...
        // Определяем роль ревьюера для этих строк и отбираем только нужные навыки
        const relevantRows = [];
        ...
        // Собираем уникальные навыки и текущие оценки по ним
        ...
        // Параллельная загрузка информации о навыках (имя, описание) с кэшированием
        const items = [];
        ...
        if (items.length > 0) {
          ...
          console.warn(`[SKILLS] No skills found for ${employee.employeeName} (role: ${detectedRole})`);
        }
