...
export const CYCLES_DB_ID = process.env.CYCLES_DB_ID;
...
function validateEnvironment() {
  const required = {
    ...
    CYCLES_DB_ID: process.env.CYCLES_DB_ID,
...
// Получение активного цикла оценки
async function getActiveCycles() {
  ...
  if (!CYCLES_DB_ID) {
    throw new Error("[CYCLE] CYCLES_DB_ID is not set");
  }

  const cycleProps = await getDbProps(CYCLES_DB_ID);
  const statusProp = cycleProps?.[CYCLE_PROP.status];
  const statusFilter = { property: CYCLE_PROP.status, select: { equals: "In Progress" } };

  if (statusProp?.type !== "select") {
    console.warn(`[CYCLE] Unexpected status property type: ${statusProp?.type}`);
  }
  ...
  if (!pages.length) {
    throw new Error("[CYCLE] No active cycle found in cycles database");
  }

  const cycles = pages.map(page => ({
    id: page.id,
    name: getTitleFromProps(page.properties || {}) || page.id
  }));

  setCached(cacheKey, cycles, DB_CACHE_TTL);
  console.log(`[CYCLE] Found ${cycles.length} active cycles`);
  return cycles;
}

async function buildCycleFilter(matrixProps) {
  const cycleProp = matrixProps?.[PROP.cycle];
  if (!cycleProp) {
    throw new Error(`[CYCLE] Cycle property "${PROP.cycle}" not found in matrix database`);
  }

  const activeCycles = await getActiveCycles();
  if (!activeCycles.length) {
    throw new Error("[CYCLE] No active cycles available");
  }

  if (cycleProp.type === "relation") {
    const filters = activeCycles.map(cycle => ({
      property: PROP.cycle,
      relation: { contains: cycle.id }
    }));
    return filters.length === 1 ? filters[0] : { or: filters };
  }

  const cycleNames = activeCycles.map(c => c.name).filter(Boolean);
  if (cycleProp.type === "select" && cycleNames.length) {
    const filters = cycleNames.map(name => ({
      property: PROP.cycle,
      select: { equals: name }
    }));
    return filters.length === 1 ? filters[0] : { or: filters };
  ...
  if (cycleProp.type === "multi_select" && cycleNames.length) {
    const filters = cycleNames.map(name => ({
      property: PROP.cycle,
  ...
  throw new Error(`[CYCLE] Unsupported cycle property type: ${cycleProp.type}`);
}
...
// ИСПРАВЛЕННАЯ функция поиска сотрудников для оценки
export async function listEvaluateesForReviewerUser(userId) {
  ...
  try {
    ...
    const cycleFilter = await buildCycleFilter(matrixProps);
    ...
    try {
      ...
      let combinedFilter = cleanFilters.length === 1 ? cleanFilters[0] : { or: cleanFilters };
      if (cycleFilter) {
        combinedFilter = { and: [cycleFilter, combinedFilter] };
      }
      ...
      for (let i = 0; i < allFilters.length; i++) {
        ...
        try {
          const filterRows = await queryAllPages({
            database_id: MATRIX_DB_ID,
            filter: cycleFilter ? { and: [cycleFilter, cleanFilter] } : cleanFilter,
            page_size: 100
          });
...
// ОПТИМИЗИРОВАННАЯ функция загрузки навыков для ревьюера
export async function fetchEmployeeSkillRowsForReviewerUser(employees, reviewerUserId) {
  ...
  try {
    ...
    // ОПТИМИЗАЦИЯ 1: Загружаем ВСЕ строки матрицы одним запросом
    console.log('[SKILLS] Loading all matrix rows in single query...');
    ...
    console.log(`[SKILLS] Loaded ${allRows.length} total matrix rows`);

    // ОПТИМИЗАЦИЯ 2: Собираем все уникальные skill IDs и их данные
    console.log('[SKILLS] Collecting all unique skills...');
    const allSkillsMap = new Map(); // skillId -> { name, description }
    const rowsByEmployeeAndRole = new Map(); // employeeId_role -> rows[]

    // Группируем строки по сотрудникам и ролям
    for (const row of allRows) {
      ...
      // Собираем skill ID для последующей загрузки
      const skillRel = props[PROP.skill]?.relation;
      const skillId = skillRel?.[0]?.id;
      if (skillId && !allSkillsMap.has(skillId)) {
        // Сохраняем props для извлечения информации о навыке
        allSkillsMap.set(skillId, { props });
      }
    }

    console.log(`[SKILLS] Found ${allSkillsMap.size} unique skills across all employees`);

    // ОПТИМИЗАЦИЯ 3: Загружаем информацию о всех навыках параллельно
    console.log('[SKILLS] Loading skill information in parallel...');
    PerformanceTracker.start('load-skill-info');
    
    const skillLoadPromises = Array.from(allSkillsMap.entries()).map(async ([skillId, data]) => {
      const skillInfo = await loadSkillInformation(skillId, data.props);
      return { skillId, info: skillInfo };
    });

    // Загружаем все навыки параллельно батчами
    const BATCH_SIZE = 20;
    const skillInfoMap = new Map();
    
    for (let i = 0; i < skillLoadPromises.length; i += BATCH_SIZE) {
      const batch = skillLoadPromises.slice(i, i + BATCH_SIZE);
      ...
      console.log(`[SKILLS] Loaded ${Math.min(i + BATCH_SIZE, skillLoadPromises.length)}/${skillLoadPromises.length} skills`);
    }
    
    PerformanceTracker.end('load-skill-info');
    console.log(`[SKILLS] All skill information loaded`);

    // ОПТИМИЗАЦИЯ 4: Собираем результат используя предзагруженные данные
    console.log('[SKILLS] Assembling final result...');
    ...
    for (const employee of employees) {
      ...
      if (employeeRows.length === 0) {
        console.warn(`[SKILLS] No rows found for employee ${employee.employeeName} with role ${employee.role}`);
        continue;
      }
      ...
      for (const row of employeeRows) {
        ...
        if (!skillInfo) {
          console.warn(`[SKILLS] No info found for skill ${skillId}`);
          continue;
        }
      ...
      if (items.length > 0) {
        ...
        console.log(`[SKILLS] Added ${items.length} skills for ${employee.employeeName} (${employee.role})`);
      }
    }
     
    const totalDuration = PerformanceTracker.end('fetch-all-skills');
    console.log(`[SKILLS] ✅ Completed loading ${result.length} skill groups in ${totalDuration}ms`);
    
    return result;
    
  } catch (error) {
    console.error(`[SKILLS] Critical error: ${error.message}`);
    PerformanceTracker.end('fetch-all-skills');
    throw error;
  }
}
