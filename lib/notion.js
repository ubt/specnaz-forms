  ...
  try {
    ...
    if (!uniqueRows.length) {
      ...
      try {
        ...
        console.log(`[EVALUATEES] Matrix contains ${sampleRows.length} total rows (sample)`);
        
        if (sampleRows.length > 0) {
          const sampleRow = sampleRows[0];
          console.log(`[EVALUATEES] Sample row structure:`, {
            id: sampleRow.id,
            properties: Object.keys(sampleRow.properties || {}),
            [PROP.selfScorer]: sampleRow.properties?.[PROP.selfScorer]?.people?.map(p => p.id) || 'empty',
            [PROP.p1Peer]: sampleRow.properties?.[PROP.p1Peer]?.people?.map(p => p.id) || 'empty',
            [PROP.p2Peer]: sampleRow.properties?.[PROP.p2Peer]?.people?.map(p => p.id) || 'empty',
            [PROP.managerScorer]: sampleRow.properties?.[PROP.managerScorer]?.people?.map(p => p.id) || 'empty',
            [PROP.employee]: sampleRow.properties?.[PROP.employee]?.relation?.map(r => r.id) || sampleRow.properties?.[PROP.employee]?.people?.map(p => p.id) || 'empty'
          });
        }
      } catch (sampleError) {
...
async function loadSkillInformation(skillId, matrixRowProps) {
  try {
    ...
    // Сначала пытаемся получить описание из rollup, так как это быстрее
    ...
    // Всегда загружаем страницу навыка для получения точного имени
    try {
      ...
      // Если описание не было найдено в rollup, ищем его на странице навыка
      if (!skillDescription) {
      ...
      if (skillName === "Неизвестный навык") {
        skillName = `Навык ${skillId.substring(0, 8)}`;
      }
    }
    ...
    return { 
      name: `Навык ${skillId.substring(0, 8)}`, 
    ...
    for (const employee of employees) {
      ...
      try {
        ...
        if (empDef?.type === "relation") {
          ...
          // Для поля people используем ID сотрудника из базы employees, а не ID пользователя
          const employeePage = await notion.pages.retrieve({ page_id: employee.employeeId });
          const peopleInEmployeeField = employeePage.properties[PROP.empAccount]?.people;
          const employeeUserId = peopleInEmployeeField?.[0]?.id;

          if (employeeUserId) {
            employeeFilter = { property: PROP.employee, people: { contains: employeeUserId } };
          } else {
             console.warn(`[SKILLS] Could not find user ID for employee page ${employee.employeeId}`);
             continue;
          }
        } else {
        ...
        if (!detectedRole) {
          ...
          if (employeeRows.length > 0) {
            const sampleRow = employeeRows[0];
            const p = sampleRow.properties;
            console.log(`[SKILLS] Sample row scorer fields:`, {
              [PROP.selfScorer]: p[PROP.selfScorer]?.people?.map(u => u.id) || 'empty',
              [PROP.p1Peer]: p[PROP.p1Peer]?.people?.map(u => u.id) || 'empty',
              [PROP.p2Peer]: p[PROP.p2Peer]?.people?.map(u => u.id) || 'empty',
              [PROP.managerScorer]: p[PROP.managerScorer]?.people?.map(u => u.id) || 'empty'
            });
          }
          continue;
        }
        ...
        for (const row of relevantRows) {
          ...
          if (!uniqueSkills.has(skillId)) {
            uniqueSkills.set(skillId, {
              pageId: row.id,
              skillId,
              current: props[field]?.number ?? null,
        ...
        for (let i = 0; i < skillEntries.length; i += BATCH_SIZE) {
          ...
          console.log(`[SKILLS] Processing batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(skillEntries.length / BATCH_SIZE)}`);

          const batchResults = await Promise.all(
            batch.map(async (entry) => {
              try {
                const skillInfo = await loadSkillInformation(entry.skillId, entry.matrixRowProps);
                return {
                  pageId: entry.pageId,
                  name: skillInfo.name,
                ...
                console.error(`[SKILLS] Error in batch for skill ${entry.skillId}:`, skillError.message);
                return null;
              }
            })
          );
