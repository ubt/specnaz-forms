...
// ИСПРАВЛЕННАЯ функция загрузки навыков
async function loadSkillInformation(skillId) {
  try {
    console.log(`[SKILL LOAD] Loading skill: ${skillId}`);
    
    // Сначала пытаемся загрузить страницу навыка напрямую
    ...
    try {
      ...
      skillName = getTitleFromProps(props) || skillName;
      ...
      console.warn(`[SKILL LOAD] Failed to load skill page ${skillId}, trying matrix fallback:`, pageError.message);
      
      // Fallback: ищем описание в матрице оценок
      try {
        ...
        if (matrixRows.length > 0) {
          ...
          if (description?.rich_text?.length > 0) {
            skillDescription = description.rich_text.map(t => t.plain_text).join("");
            // Пытаемся извлечь название из начала описания
            const lines = skillDescription.split('\n');
            if (lines.length > 0 && lines[0].length < 100) {
              skillName = lines[0].trim() || skillName;
            }
          }
          console.log(`[SKILL LOAD] ✅ Used matrix fallback for skill: ${skillName}`);
        }
      } catch (fallbackError) {
        console.warn(`[SKILL LOAD] Matrix fallback also failed for ${skillId}:`, fallbackError.message);
      }
    }
    ...
    return { 
      name: "Навык (ошибка загрузки)", 
      description: `Ошибка: ${error.message}` 
    };
  }
}

// ИСПРАВЛЕННАЯ основная функция загрузки навыков для ревьюера
    ...
    for (const employee of employees) {
      ...
      try {
        ...
        for (const skillEntry of skillEntries) {
          try {
            const skillInfo = await loadSkillInformation(skillEntry.skillId);
            ...
            console.log(`[SKILLS] Added skill: ${skillInfo.name} (current: ${skillEntry.current})`);
            ...
            items.push({
              pageId: skillEntry.pageId,
              name: "Навык (ошибка загрузки)",
              description: `Ошибка: ${skillError.message}`,
              current: skillEntry.current,
              comment: ""
            });
          }
