...
export const CONFIG = {
  ...
  // ИСПРАВЛЕНО: Учитываем лимит subrequests Cloudflare Workers (50)
  BATCH: {
    MAX_OPERATIONS: 500,      // Максимум операций в одном batch (разбивается на jobs)
    DEFAULT_BATCH_SIZE: 20,   // КРИТИЧНО: Безопасный размер батча (меньше половины лимита)
    MAX_BATCH_SIZE: 25,       // КРИТИЧНО: Максимальный размер батча с большим запасом
    MAX_CONCURRENCY: 1,       // ИЗМЕНЕНО: Последовательная обработка
    DEFAULT_CONCURRENCY: 1,   // ИЗМЕНЕНО: Последовательная обработка
    MIN_RATE_LIMIT_DELAY: 2500, // УВЕЛИЧЕНО: Минимальная задержка между батчами
    MAX_RETRIES: 3,           // Максимум повторов
  },

  // Настройки для прямой обработки (без KV)
  // ИСПРАВЛЕНО: Строгий лимит для одного worker invocation
  // Оставляем большой запас от лимита в 50 subrequests
  DIRECT_PROCESSING: {
    MAX_OPERATIONS: 25,       // КРИТИЧНО: Безопасный лимит с запасом (50 - 25 = запас)
    MAX_OPERATION_SIZE: 8000, // Максимальный размер операции
  },
  ...
  // ИСПРАВЛЕНО: Уменьшено для соблюдения лимита subrequests
  PARALLEL: {
    SKILL_LOAD_BATCH: 15,     // УМЕНЬШЕНО: Параллельная загрузка навыков
    EMPLOYEE_LOAD_BATCH: 10,  // УМЕНЬШЕНО: Параллельная загрузка сотрудников
  },
  
  // Лимиты Cloudflare
  CLOUDFLARE: {
    SUBREQUEST_LIMIT: CLOUDFLARE_SUBREQUEST_LIMIT,
    SAFE_SUBREQUEST_LIMIT: CLOUDFLARE_SUBREQUEST_LIMIT - 5, // С запасом
  }
};
