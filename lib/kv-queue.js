...
// УЛУЧШЕННОЕ получение статуса batch операций с обработкой ошибок
export async function getKVBatchStatus(jobIds) {
  try {
    ...
    // Обрабатываем по одному с задержками для стабильности
...
// УЛУЧШЕННОЕ получение результатов выполнения
export async function getKVBatchResults(jobIds) {
  try {
    ...
    // Обрабатываем по одному для стабильности
    for (const jobId of jobIds) {
      try {
    ...
    console.log(`[KV RESULTS] Получено ${allResults.length} результатов`);
    return allResults;
...
export class NotionBatchProcessor {
  constructor(notionClient, options = {}) {
    ...
    this.options = {
      batchSize: Math.min(options.batchSize || 20, 25), // Ограничение для стабильности
