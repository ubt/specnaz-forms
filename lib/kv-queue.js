...
export function initKV(env = {}) {
  KV_NAMESPACE = env?.NOTION_QUEUE_KV || (typeof NOTION_QUEUE_KV !== 'undefined' ? NOTION_QUEUE_KV : null);
  isKVAvailable = Boolean(KV_NAMESPACE);

 if (isKVAvailable) {
    console.log('[KV] Cloudflare KV подключено успешно');
  } else {
    console.warn(`[KV] Cloudflare KV недоступно. Binding NOTION_QUEUE_KV ${env?.NOTION_QUEUE_KV ? 'есть, но не инициализировалось' : 'не найдено в context.env'}`);
  }
}
}
